...
const _default = {
    ...
    load_: async function(app, options, name) {
        const { chunkSize, maxChunkOverlap, overlapSeparators, encodingForModel: _encodingForModel } = app.featureConfig(options ?? {}, {
            schema: {
                chunkSize: {
                    type: 'integer',
                    default: 4095
        ...
        if (maxChunkOverlap > chunkSize) {
            throw new _types.InvalidConfiguration('maxChunkOverlap must be less than chunkSize', app, 'aiTextSplitter.maxChunkOverlap');
        }
        ...
        const service = {
            ...
            split: (text, chunkHeaderOptions)=>{
                const { chunkSize: __chunkSize, maxChunkOverlap: _maxChunkOverlap, chunkHeader, chunkOverlapHeader, appendChunkOverlapHeader } = _types.Types.OBJECT.sanitize(chunkHeaderOptions ?? {}, {
                    schema: {
                        chunkSize: {
                            type: 'integer',
                            default: chunkSize
                ...
                if (_chunkSize2 < _maxChunkOverlap) {
                    throw new _types.ValidationError('"chunkHeader" or "chunkOverlapHeader" too long', {
                        chunkHeader,
                        chunkOverlapHeader,
                        appendChunkOverlapHeader,
                ...
                while(remainingTokens.length > 0){
                    ...
                    if (remainingTokens.length > _chunkSize) {
                        ...
                        if (bestSeparatorInOverlap !== -1) {
                            chunkEnd = overlapStart + tokenizer.encode(overlapText.slice(0, bestSeparatorInOverlap)).length;
                        }
                    }
...
//# sourceMappingURL=aiTextSplitter.js.map