...
class RelationalConnector extends _Connector.default {
     ...
     */ buildQuery(model, { $relation, $select, $where, $groupBy, $orderBy, $offset, $limit, $totalCount, $key }) {
        ...
        if (needDistinctForLimit) {
            ...
            const keySqlAnchor = (0, _numbertoletter.default)(keySqlAliasIndex);
     ...
     */ _joinAssociation(assocInfo, anchor, joinings, parentAliasKey, aliasMap, startId, params) {
        ...
        if ($relation) {
            ...
            joinings.push(`${joinType} ${this.escapeId(entity)} ${alias} ON ${this._joinCondition(on, params, null, parentAliasKey, aliasMap)}`);
     ...
     */ _joinCondition(condition, params, joinOperator, mainEntity, aliasMap) {
        if (Array.isArray(condition)) {
            ...
            return condition.map((c)=>'(' + this._joinCondition(c, params, null, mainEntity, aliasMap) + ')').join(` ${joinOperator} `);
        }
     ...
     */ _wrapCondition(fieldName, value, params, mainEntity, aliasMap, inject) {
        ...
        if (Array.isArray(value)) {
            return this._wrapCondition(fieldName, {
                $in: value
            }, params, mainEntity, aliasMap, inject);
        }
        if ((0, _utils.isPlainObject)(value)) {
            if (value.$xr) {
                    ...
                    if (k && k[0] === '$') {
                        ...
                        switch(k){
                            ...
                            case '$lessThanOrEqual':
                                ...
                                return this._escapeIdWithAlias(fieldName, mainEntity, aliasMap) + ` <= ${this._packValue(v, params, mainEntity, aliasMap)}`;
                            case '$in':
                                if ((0, _utils.isPlainObject)(v) && v.$xr === 'DataSet') {
                                    ...
                                    if (!Array.isArray(v)) {
                                        throw new Error('The value should be a dataset or an array when using "$in" operator.');
                                    }
                                    if (inject) {
                                        return this._escapeIdWithAlias(fieldName, mainEntity, aliasMap) + ` IN (${v})`;
                                    }
                                    params.push(v);
                                    return this._escapeIdWithAlias(fieldName, mainEntity, aliasMap) + this.specInClause(params.length);
                                }
                            case '$nin':
                            case '$notIn':
                                if ((0, _utils.isPlainObject)(v) && v.$xr === 'DataSet') {
                                    ...
                                    if (inject) {
                                        return this._escapeIdWithAlias(fieldName, mainEntity, aliasMap) + ` NOT IN (${v})`;
                                    }
                                    params.push(v);
                                    return this._escapeIdWithAlias(fieldName, mainEntity, aliasMap) + this.specNotInClause(params.length);
                                }
                            case '$startWith':
                            case '$startsWith':
                                if (typeof v !== 'string') {
     ...
     */ _buildColumns(columns, params, mainEntity, aliasMap) {
        return _utils._.map(_utils._.castArray(columns), (col)=>this._buildColumn(col, params, mainEntity, aliasMap)).join(', ');
    }
    /**
     *
     ...
     */ _buildColumn(col, params, mainEntity, aliasMap) {
        if (typeof col === 'string') {
            ...
            return (0, _Rules.isRawSql)(col) ? (0, _Rules.extractRawSql)(col) : this._escapeIdWithAlias(col, mainEntity, aliasMap);
        }
        if (typeof col === 'number') {
            return col;
        }
        if ((0, _utils.isPlainObject)(col)) {
            if (col.alias) {
                ...
                return this._buildColumn(_utils._.omit(col, [
                    'alias'
                ]), params, mainEntity, aliasMap) + ' AS ' + this.escapeId(alias);
            }
     ...
     */ _buildGroupByColumn(groupBy, mainEntity, aliasMap) {
        if (typeof groupBy === 'string') {
            return (0, _Rules.isRawSql)(groupBy) ? (0, _Rules.extractRawSql)(groupBy) : this._escapeIdWithAlias(groupBy, mainEntity, aliasMap);
        }
...
const _default = RelationalConnector;
