...
module.exports = Base => class extends Base {
  ...
  static validatePassword(user, password) {
    ...
    const hashedPassword = this.$hashPassword(password, user.passwordSalt);
  ...
  static async loginUser_(loggedInUser, requireRoles, forUsage) {
    const user = _.pick(loggedInUser, ['id', 'username', 'mobile', 'email', 'avatar', 'name']);
    ...
    if (forUsage) {
      roles = roles.filter(role => role[':role'].usage === forUsage && !role[':role'].isSpecific);
  ...
  static async addUnverifiedUser_(userInput, connOpts) {
    ...
    return this.db.doTransaction_(async connOpts => {
      ...
      if (users.length > 0) {
        ...
        throw new ValidationError(`User with the same "${whereSame}" already exists.`);
      }
      ...
      return this.create_(_userInput, {
  ...
  static async doesUserExist_(username, excludeAgent) {
    ...
    if (excludeAgent) {
      return user && _.every(user[':roles'], role => role.role !== 'AGENT');
    }
  ...
  static async checkUserWithAnyOf_(mobile, email) {
    ...
    if (users.length > 1) {
      throw new BadRequest('The specified mobile and email link to two different users.');
    }
  ...
  static async activate_(verifyInfo, password, connOpts) {
    const payload = verifyInfo.payload && JSON.parse(verifyInfo.payload);
    const userData = {
      status: 'active'
    };
    ...
    const user = await this.updateOne_(userData, {
      $query: {
        id: verifyInfo.user
      },
      $retrieveUpdated: {
  ...
  static async grantRoles_(user, roles, payload, connOpts) {
    ...
    return this.db.doTransaction_(async connOpts => {
      ...
      const currentRoles = new Set([user[':roles'].map(role => role.role)]);
      roles.forEach(role => {
        currentRoles.add(role);
      });
      await this.db.connector.insertMany_('userRole', ['user', 'role'], Array.from(roles).map(role => [user.id, role]), { ...connOpts,
        insertIgnore: true
      });

      if (userRoleTypes.length > 0) {
        await eachAsync_(userRoleTypes, role => this.grantSpecialRole_(user, role, payload, connOpts));
      }
    }, null, connOpts);
  }
