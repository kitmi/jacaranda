...
class MySQLModeler {
    ...
    _oolConditionToQueryCondition(context, oolCon) {
        ...
        } else if (oolCon.$xt === "UnaryExpression") {
            let arg;
        ...
        } else if (oolCon.$xt === "LogicalExpression") {
            switch (oolCon.operator) {
                case "and":
                    return {
                        $and: [
    ...
    static _orderByToSql(schema, doc, ool) {
        return (
            MySQLModeler.oolToSql(schema, doc, { $xt: "ObjectReference", name: ool.field }) +
            (ool.ascend ? "" : " DESC")
        );
    }
    ...
    static defaultValue(info, type) {
        ...
        if (!info.optional) {
            if (info.hasOwnProperty("default")) {
                ...
                if (typeof defaultValue === "object" && defaultValue.$xr === "SymbolToken") {
