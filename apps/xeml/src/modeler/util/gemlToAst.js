...
const GemlTypes = require('../../lang/XemlTypes.js');
const { isDotSeparateName, extractDotSeparateName, extractReferenceBaseName } = require('../../lang/XemlUtils.js');
...
function compileAdHocValidator(topoId, value, functor, compileContext) {
    assert: functor.$xt === GemlTypes.Modifier.VALIDATOR;        
...
function compileModifier(topoId, value, functor, compileContext) {
    ...
    if (functor.$xt === GemlTypes.Modifier.ACTIVATOR) { 
    ...
    if (functor.$xt === GemlTypes.Modifier.ACTIVATOR) {            
        ...
        let arg0 = value;
        if (!isTopLevelBlock(topoId) && _.isPlainObject(value) && value.$xt === 'ObjectReference' && value.name.startsWith('latest.')) {
            //let existingRef =            
    ...
    if (isTopLevelBlock(topoId)) {
        ...
        if (!isDotSeparateName(value.name) && compileContext.variables[value.name] && functor.$xt !== GemlTypes.Modifier.VALIDATOR) {
        ...
        addCodeBlock(compileContext, topoId, {
            type: OOL_MODIFIER_CODE_FLAG[functor.$xt],
...
function translateModifier(functor, compileContext, args) {
    ...
    if (isDotSeparateName(functor.name)) {
        ...
        fileName = './' + OOL_MODIFIER_PATH[functor.$xt] + '/' + refEntityName + '-' + functionName + '.js';
        ...
        let builtins = OOL_MODIFIER_BUILTIN[functor.$xt];

        if (!(functionName in builtins)) {
            fileName = './' + OOL_MODIFIER_PATH[functor.$xt] + '/' + compileContext.moduleName + '-' + functionName + '.js';
...
function compilePipedValue(startTopoId, varOol, compileContext) {
    ...
    varOol.modifiers.forEach(modifier => {
        let modifierStartTopoId = createTopoId(compileContext, startTopoId + OOL_MODIFIER_OP[modifier.$xt] + modifier.name);
...
function compileVariableReference(startTopoId, varOol, compileContext) {
    pre: _.isPlainObject(varOol) && varOol.$xt === 'ObjectReference';
...
function compileExceptionalReturn(oolNode, compileContext, dependency) {
    pre: (_.isPlainObject(oolNode) && oolNode.$xt === 'ReturnExpression');
