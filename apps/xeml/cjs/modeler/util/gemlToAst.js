    ...
    if (_.isPlainObject(test)) {
        if (test.oolType === 'ValidateExpression') {
            ...
            */ return endTopoId;
            ...
            compileContext.astMap[endTopoId] = JsLang.astBinExp(getCodeRepresentationOf(lastLeftId, compileContext), op1, getCodeRepresentationOf(lastRightId, compileContext));
            return endTopoId;
 ...
 */ function compileAdHocValidator(topoId, value, functor, compileContext) {
    ...
    let arg0 = value;
    compileContext.astMap[topoId] = JsLang.astCall('Validators.' + functor.name, [
        arg0
    ].concat(callArgs));
    return topoId;
}
 ...
 */ function compileModifier(topoId, value, functor, compileContext) {
    ...
    if (functor.oolType === GemlTypes.Modifier.ACTIVATOR) {
        ...
        declareParams = translateFunctionParams(_.isEmpty(functor.args) ? [
            value
        ] : [
            value
        ].concat(functor.args));
    ...
    if (functor.oolType === GemlTypes.Modifier.ACTIVATOR) {
        compileContext.astMap[topoId] = JsLang.astAwait(functorId, [
        ...
        if (!isTopLevelBlock(topoId) && _.isPlainObject(value) && value.oolType === 'ObjectReference' && value.name.startsWith('latest.')) {
            //let existingRef =            
            arg0 = JsLang.astConditional(JsLang.astCall('latest.hasOwnProperty', [
                extractReferenceBaseName(value.name)
            ]), /** test */ value, /** consequent */ replaceVarRefScope(value, 'existing'));
        }
        compileContext.astMap[topoId] = JsLang.astCall(functorId, [
            arg0
        ].concat(callArgs));
    }
...
function getCodeRepresentationOf(topoId, compileContext) {
    ...
    if (ast.type === 'MemberExpression' && ast.object.name === 'latest') {
        return JsLang.astConditional(JsLang.astCall('latest.hasOwnProperty', [
            ast.property.value
        ]), /** test */ ast, /** consequent */ {
            ...ast,
            object: {
